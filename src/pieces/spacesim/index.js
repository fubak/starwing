...
// radar, speed lines, laggy chase camera with roll, nebula sky, colour grading.
import * as THREE from 'three';
import { ShaderPass } from 'three/addons/postprocessing/ShaderPass.js';
import { buildNebula, buildStars, buildSun, buildPlanet, SUN_DIR } from './sky.js';
...
export async function create(ctx) {
  const { scene, camera, renderer, composer, bloom, input, ui, rng, size, audio } = ctx;
  const disposables = [];

  // ---------------- sky + lighting
  // nebula is baked once into a cube map (the procedural shader is far too heavy per-frame)
  const nebula = buildNebula();
  const skyCube = bakeCube(renderer, nebula, 512);
  nebula.geometry.dispose(); nebula.material.dispose();
  scene.background = skyCube.texture;
  const sky = new THREE.Group();
  const stars = buildStars(rng); const sun = buildSun();
  stars.material.uniforms.uPR.value = renderer.getPixelRatio();
  sky.add(stars, sun);
  scene.add(sky);

  // environment map from the nebula for real specular/fresnel response on the ship + rocks
  const pmrem = new THREE.PMREMGenerator(renderer);
  const envRT = pmrem.fromCubemap(skyCube.texture);
  ...
  scene.add(sunLight);
  const hemi = new THREE.HemisphereLight(0x3a6a9a, 0x3a1838, 0.55);
  scene.add(hemi);
  const rim = new THREE.DirectionalLight(0x8a4dff, 0.9); // nebula bounce from the opposite side
  rim.position.set(0.6, -0.3, 0.7).multiplyScalar(100);
  scene.add(rim);

  const planet = buildPlanet(renderer, { radius: 1050, position: new THREE.Vector3(1700, -600, -2700) });
  scene.add(planet.group);

  // ---------------- asteroid belt
  const belt = buildAsteroidBelt(rng, { count: 720, extent: 560, thickness: 150, envMap });
  ...
  try {
    ...
    if (typeof shipMod.buildArwing === 'function') {
      const r = shipMod.buildArwing({ THREE, envMap });
      const obj = r?.isObject3D ? r : r?.group ?? r?.mesh ?? r?.object;
      if (obj?.isObject3D) arwing = obj;
    }
  } catch (e) { arwing = null; }
  if (!arwing) arwing = buildFallbackArwing({ envMap });
  ...
  const SPEED = { cruise: 55, boost: 125, brake: 22 };

  // ---------------- drones (targets)
  const drones = [];
  const droneMatBody = new THREE.MeshPhysicalMaterial({ color: 0x8a94a8, metalness: 0.7, roughness: 0.35, envMap, envMapIntensity: 1.0, clearcoat: 0.4 });
  const droneMatRed = new THREE.MeshStandardMaterial({ color: 0xff3b2f, emissive: 0xff2a1a, emissiveIntensity: 1.6, roughness: 0.4, metalness: 0.2 });
  const droneGeoBody = new THREE.OctahedronGeometry(1.6, 0); droneGeoBody.scale(1, 0.55, 1.6);
  const droneGeoRing = new THREE.TorusGeometry(2.1, 0.18, 8, 24);
  const droneGeoEye = new THREE.SphereGeometry(0.5, 12, 8);
  ...
  for (let i = 0; i < 6; i++) {
    ...
    const body = new THREE.Mesh(droneGeoBody, droneMatBody);
    const ring = new THREE.Mesh(droneGeoRing, droneMatBody); ring.rotation.x = Math.PI / 2; ring.scale.y = 0.6;
    const eye = new THREE.Mesh(droneGeoEye, droneMatRed); eye.position.z = -1.4;
    const wingL = new THREE.Mesh(new THREE.BoxGeometry(2.6, 0.12, 1.1), droneMatBody); wingL.position.set(-2.1, 0, 0.6); wingL.rotation.z = 0.35;
    const wingR = wingL.clone(); wingR.position.x = 2.1; wingR.rotation.z = -0.35;
    g.add(body, ring, eye, wingL, wingR);
    g.scale.setScalar(1.4);
    scene.add(g);
    drones.push({
      obj: g, name: droneNames[i], phase: rng.range(0, 6.28), r: rng.range(160, 380), spd: rng.range(0.12, 0.28) * rng.sign(),
      tilt: rng.range(-0.6, 0.6), hp: 3, dead: 0, prev: new THREE.Vector3(),
    });
  }

  // ---------------- lasers
  const boltGeo = new THREE.CapsuleGeometry(0.16, 3.4, 4, 8); boltGeo.rotateX(Math.PI / 2);
  const boltMat = new THREE.MeshBasicMaterial({ color: 0x5cff6a, toneMapped: false, transparent: true, blending: THREE.AdditiveBlending, depthWrite: false });
  boltMat.color.multiplyScalar(3);
  const boltCore = new THREE.MeshBasicMaterial({ color: 0xffffff, toneMapped: false });
  const bolts = [];
  for (let i = 0; i < 24; i++) {
    const m = new THREE.Mesh(boltGeo, boltMat);
    const c = new THREE.Mesh(boltGeo, boltCore); c.scale.set(0.45, 0.45, 0.9); m.add(c);
    m.visible = false; scene.add(m);
    bolts.push({ mesh: m, vel: new THREE.Vector3(), life: 0 });
  }
  const muzzleL = new THREE.Vector3(-3.9, -1.0, -1.2), muzzleR = new THREE.Vector3(3.9, -1.0, -1.2);
  let muzzleSide = 0;
  function fire() {
    ...
    b.mesh.quaternion.copy(ship.quat);
    // converge bolts toward the reticle point ahead
    _v.copy(FWD).applyQuaternion(ship.quat).multiplyScalar(160).add(ship.pos).sub(b.mesh.position).normalize();
    b.vel.copy(_v).multiplyScalar(420 + ship.speed);
  ...
  for (let i = 0; i < 8; i++) {
    ...
    const ring = new THREE.Mesh(new THREE.RingGeometry(0.8, 1, 48), new THREE.MeshBasicMaterial({ color: 0xffc070, transparent: true, blending: THREE.AdditiveBlending, depthWrite: false, side: THREE.DoubleSide, toneMapped: false }));
    s.visible = false; ring.visible = false; scene.add(s, ring);
    explosions.push({ s, ring, t: 1, dur: 0.9 });
  }
  function explode(p, scale = 1) {
    const e = explosions.find((x) => x.t >= x.dur) ?? explosions[0];
    e.t = 0; e.scale = scale; e.s.position.copy(p); e.ring.position.copy(p); e.ring.quaternion.copy(camera.quaternion);
    e.s.visible = e.ring.visible = true;
  ...
  for (let i = 0; i < N_LINES; i++) {
    ...
    lineData.push({ x: Math.cos(a) * r, y: Math.sin(a) * r, z: rng.range(-60, -5), len: rng.range(3, 9) });
  }
  const lineGeo = new THREE.BufferGeometry(); lineGeo.setAttribute('position', new THREE.BufferAttribute(linePos, 3));
  const lineMat = new THREE.LineBasicMaterial({ color: 0x9fdfff, transparent: true, opacity: 0, blending: THREE.AdditiveBlending, depthWrite: false, toneMapped: false });
  ...
  // ---------------- post: colour grade + vignette (inserted before the output pass)
  const gradePass = new ShaderPass({
    uniforms: { tDiffuse: { value: null }, uBoost: { value: 0 }, uFlash: { value: 0 } },
    vertexShader: `varying vec2 vUv; void main(){ vUv = uv; gl_Position = projectionMatrix*modelViewMatrix*vec4(position,1.0); }`,
    fragmentShader: /* glsl */ `
      uniform sampler2D tDiffuse; uniform float uBoost; uniform float uFlash; varying vec2 vUv;
      void main(){
        vec2 uv = vUv; vec2 d = uv - 0.5;
        float ca = 0.0025 + uBoost * 0.006;
        vec3 c;
        c.r = texture2D(tDiffuse, uv + d * ca).r;
        c.g = texture2D(tDiffuse, uv).g;
        c.b = texture2D(tDiffuse, uv - d * ca).b;
        // grade: cool shadows, warm highlights, slight contrast + saturation
        float l = dot(c, vec3(0.2126, 0.7152, 0.0722));
        c = mix(vec3(l), c, 1.12);
        c += vec3(-0.004, 0.0, 0.012) * (1.0 - smoothstep(0.0, 0.5, l));
        c *= vec3(1.03, 1.0, 0.98);
        float vig = smoothstep(0.95, 0.35, length(d) * (1.0 + uBoost * 0.25));
        c *= mix(0.55, 1.0, vig);
        c += uFlash * vec3(1.0, 0.9, 0.8);
        gl_FragColor = vec4(c, 1.0);
      }`,
  });
  composer.insertPass(gradePass, composer.passes.length - 1);
  const bloomPrev = { strength: bloom.strength, radius: bloom.radius, threshold: bloom.threshold };
  bloom.strength = 0.85; bloom.radius = 0.45; bloom.threshold = 0.72;
  ...
  // ---------------- camera
  camera.fov = 62; camera.near = 0.3; camera.far = 9000; camera.updateProjectionMatrix();
  const camQuat = new THREE.Quaternion();
  const camPos = new THREE.Vector3(0, 3, 14);
  let camShake = 0, flash = 0;
  const camOffset = new THREE.Vector3(0, 2.6, 11.5);
  ...
  input.script = (t) => {
    ...
    let x = 0, y = 0;
    const s = t % 24;
    if (s < 3) { x = Math.sin(t * 1.2) * 0.5; y = Math.sin(t * 0.7) * 0.25; if (s > 1.2) B.push('fire'); }
  ...
  function startManeuver(type) {
    ...
    ship.maneuver = { type, t: 0, dur, q0: ship.quat.clone(), applied: 0 };
  ...
  function maneuverPose(m, k) {
    // returns {pitch, roll, bankVis} angles at normalised progress k with anticipation + overshoot
  ...
  let time = 0, lockedIdx = 0;

  function update(dt, t) {
    time = t; hud.state.time = t;
    if (size.x !== hud.canvas.width / Math.min(devicePixelRatio || 1, 2) || size.y !== hud.canvas.height / Math.min(devicePixelRatio || 1, 2)) hud.resize(size.x, size.y);
    ...
    ship.boostMeter = THREE.MathUtils.clamp(ship.boostMeter + (boostHeld ? -0.35 : 0.22) * dt, 0, 1);
    ...
    if (m) {
      ...
      const rollIn = 0;
      ship.rollRate += (rollIn - ship.rollRate) * damp(5, dt);
    ...
    shipBank.rotation.set(0, 0, ship.bank);
    // subtle hover bob + pitch anticipation from stick
    shipBank.rotation.x = ship.pitchRate * 0.12;
    ...
    for (const b of bolts) {
      ...
      for (const d of drones) {
        ...
        if (b.mesh.position.distanceToSquared(d.obj.position) < 5.5 * 5.5) {
          b.mesh.visible = false; d.hp--; flash = Math.max(flash, 0.05);
          explode(b.mesh.position, 0.35);
          if (d.hp <= 0) { d.dead = 4; d.obj.visible = false; explode(d.obj.position, 1.6); camShake = Math.max(camShake, 0.35); flash = 0.25; }
          break;
        }
      }
    }
    ...
    lockedIdx = best;
    hud.state.targets = drones.filter((d) => d.dead <= 0).map((d, i) => ({ pos: d.obj.position, dist: d.obj.position.distanceTo(ship.pos), name: d.name, locked: drones.indexOf(d) === lockedIdx }));
    ...
    for (const e of explosions) {
      ...
      if (e.t >= e.dur) e.s.visible = e.ring.visible = false;
    }

    // ---- belt wrap + spin, sky follows camera
    ...
    _v.copy(camOffset); _v.z += ship.boost * 2.5; _v.applyQuaternion(camQuat).add(ship.pos);
    camPos.lerp(_v, damp(m ? 6 : 9, dt));
    camShake *= 1 - damp(4, dt); flash *= 1 - damp(10, dt);
    const sh = camShake + ship.boost * 0.04;
    camera.position.copy(camPos);
    camera.position.x += (rng.next() - 0.5) * sh; camera.position.y += (rng.next() - 0.5) * sh;
    _v.copy(FWD).applyQuaternion(ship.quat).multiplyScalar(40).add(ship.pos);
    camera.up.copy(UP).applyQuaternion(camQuat);
    camera.lookAt(_v);
    const fovT = 62 + ship.boost * 13 + (brakeHeld ? -4 : 0);
    ...
    lineMat.opacity = lineAlpha * 0.55;
    if (lineAlpha > 0.001) {
    ...
    // ---- post + hud
    gradePass.uniforms.uBoost.value = ship.boost; gradePass.uniforms.uFlash.value = flash;
  ...
  function dispose() {
    composer.removePass(gradePass); gradePass.dispose?.();
    bloom.strength = bloomPrev.strength; bloom.radius = bloomPrev.radius; bloom.threshold = bloomPrev.threshold;
    hud.dispose();
    ...
    scene.environment = null; scene.background = null; envRT.dispose(); skyCube.dispose(); planet.dispose();
    belt.dispose();
