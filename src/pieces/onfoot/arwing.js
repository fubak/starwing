...
export function buildDockedArwing(scale = 1.55, groundY = 1.6) {
  ...
  for (const [x, z] of [[0, -2.6 * scale], [-0.85 * scale, 1.1 * scale], [0.85 * scale, 1.1 * scale]]) {
    const strut = new THREE.Mesh(new THREE.CylinderGeometry(0.07, 0.09, len, 10), strutMat); strut.position.set(x, hullBottom - len / 2, z); strut.castShadow = true; g.add(strut);
    ...
    const pad = new THREE.Mesh(new THREE.CylinderGeometry(0.34, 0.4, 0.14, 14), strutMat); pad.position.set(x, -groundY + 0.07, z); pad.castShadow = true; pad.receiveShadow = true; g.add(pad);
